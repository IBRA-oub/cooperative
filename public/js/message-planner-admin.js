...
function sendMessage() {
    ...
    $.ajax({
        url: '/planner-message-admin-post', 
        type: 'POST',
        data: formData,
        ...
        success: function(response) {
            ...
            messages.forEach(function(message) {
                ...
                if (message.sender === 'planner') {
                    ...
                    '<span class="px-4 py-2 rounded-lg rounded-br-none bg-green-300 text-white">';
                } 
                ...
                var existingMessage = $('#messagesPlannerAdmin').find('#message_' + message.id);
                if (existingMessage.length > 0) {
                    ...
                    $('#messagesPlannerAdmin').append('<div  id="message_' + message.id + '">' + messageHTML + '</div>');
                }
            });
        },
        error: function(xhr, status, error) {
